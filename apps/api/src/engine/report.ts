...
export function fmtKpiValue(k: A.KpiResult, currency = DEFAULT_CURRENCY): string {
  if (k.format === "money") return A.fmtMoney(k.value, currency);
...
export function composeReport(pack: IndustryPack, rows: Row[], schema: SchemaMap, cal: CalendarConfig = DEFAULT_CALENDAR, currency = DEFAULT_CURRENCY): ReportContent {
