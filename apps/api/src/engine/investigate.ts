...
export function investigate(rows: Row[], s: SchemaMap, metricId: string, packId?: string): Investigation {
  ...
  for (const d of drivers.slice(0, 3)) {
    ...
    if (!top) continue;
    claims.push({ kind: "driver", metric: metric.id, period: "current vs previous half", dimension: d.dimension, detail: `${top.name} contribution ${fmt(top.value)}`, rows: d.rows, value: top.value });
  }
  ...
  for (const f of (correlation?.factors ?? []).slice(0, 3)) {
    ...
    claims.push({ kind: "correlation", metric: metric.id, period: "monthly", dimension: f.column, detail: `${f.column} moves with ${metric.id}`, rows: rows.length, value: f.correlation });
  }
