...
export function investigate(rows: Row[], s: SchemaMap, metricId: string, packId?: string): Investigation {
  ...
  for (const d of drivers.slice(0, 3)) {
    ...
    if (!top) continue;
    claims.push({ kind: "driver", metric: metric.id, period: "current vs previous half", dimension: d.dimension ?? undefined, detail: `${top.label} contribution ${fmt(top.delta)}`, rows: rows.length, value: top.delta });
  }
  ...
  for (const a of (anomalies?.points ?? []).slice(0, 2)) {
    ...
    claims.push({ kind: "anomaly", metric: metric.id, period: a.period, detail: `${a.period} was anomalous (${fmt(a.deviation)} vs median ${fmt(a.expected)})`, rows: rows.length, value: a.deviation });
  }
...
function buildNarrative(metric: PackMetric, totalDelta: number, changePct: number | null, drivers: DriverResult[], decomposition: ReturnType<typeof decomposePriceVolumeMix> | null, correlation: CorrelationResult | null, anomalies: AnomalyResult | null): string {
