...
// AI question understanding — the "translate-only" layer. Claude turns a free-form
// question into a structured Intent; the deterministic engine (engine/intent.ts →
// runIntent) then computes every number. Claude is sent ONLY the question and the
...
const client = env.anthropicApiKey
  ? new Anthropic({ apiKey: env.anthropicApiKey, timeout: 12_000, maxRetries: 1 })
  : null;

const METRIC_ENUM = ["revenue", "profit", "quantity", "orders"] as const;

// Claude emits this compact shape; we widen it to a full Intent below.
...
const emitIntentTool: Anthropic.Tool = {
  name: "emit_intent",
  description: "Record the structured analytics intent for the user's question.",
  input_schema: {
    type: "object",
    properties: {
...
const SYSTEM = `You translate a business user's natural-language question about their dataset into a single structured analytics intent. You never see the data itself, only its columns. Always call the emit_intent tool exactly once. Choose the intent that best matches the question:
...
export async function llmParseIntent(question: string, schema: SchemaMap): Promise<Intent | null> {
  ...
  try {
    ...
    const call = res.content.find((b): b is Anthropic.ToolUseBlock => b.type === "tool_use" && b.name === "emit_intent");
    if (!call) return null;
    const e = Emitted.parse(call.input);
