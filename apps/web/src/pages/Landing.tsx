...
import {
  ClipboardCheck, BarChart3, MessagesSquare, TrendingUp, FileText, Bell,
    ...
    <header className="sticky top-0 z-30 border-b border-rule bg-surface">
      <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-5">
        <div className="flex items-center gap-2 text-ink">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-accent text-accent-fg"><ClipboardCheck className="h-[18px] w-[18px]" /></div>
...
function PreviewMock() {
  ...
  return (
    <motion.div
      className="relative overflow-hidden rounded-xl border border-rule bg-surface p-4 shadow-card"
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-2">
        {kpis.map((k, i) => (
          <motion.div
            key={k.label}
            className="rounded-lg border border-rule-soft bg-sunken p-3"
            initial={{ opacity: 0, y: 8 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: DUR.base, ease: EASE, delay: 0.35 + i * 0.08 }}
          >
            <div className="label">{k.label}</div>
            <div className="mt-1 text-data tabular-nums text-ink">
              <AnimatedNumber value={k.value} format={k.format} duration={1.4} />
            </div>
            <div className="font-mono text-label text-pos">{k.change}</div>
          </motion.div>
        ))}
      </div>

      <div className="mt-3 rounded-lg border border-rule-soft bg-sunken p-3">
        <div className="label mb-2">Revenue by category</div>
...
export default function Landing() {
  ...
  return (
    <div className="min-h-full bg-canvas text-ink">
      {/* Hero (always dark) */}
      <div ref={heroRef} className="relative overflow-hidden border-b border-rule bg-surface">
        <motion.div
          style={{ y: gridY }}
        ...
        <div className="relative">
          ...
          <div className="mx-auto grid max-w-6xl items-center gap-10 px-5 py-16 lg:grid-cols-2 lg:py-24">
            <Stagger inView={false} stagger={0.09}>
              <StaggerItem>
                <span className="inline-flex items-center gap-1.5 rounded-full border border-rule bg-sunken px-3 py-1 text-body-sm font-medium text-ink-faint">
              ...
              <StaggerItem>
                <p className="mt-5 max-w-md text-lg text-ink-faint">
              ...
              <StaggerItem>
                <p className="mt-4 text-sm text-ink-faint">Free plan · no credit card · load sample data in one click.</p>
              </StaggerItem>
            </Stagger>
            <PreviewMock />
          </div>
        ...
        <Stagger className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {TRUST.map((t) => (
            <HoverCard key={t.title} className="rounded-2xl border border-rule p-5">
              ...
              <p className="mt-1 text-sm text-ink-faint">{t.body}</p>
            </HoverCard>
          ))}
        </Stagger>
      </section>
      ...
      {/* Features */}
      <section className="border-y border-rule bg-sunken py-16">
        ...
        <Stagger stagger={0.04} className="mt-8 flex flex-wrap justify-center gap-3">
          {industries.map((i) => (
            <motion.div key={i.key} variants={staggerItem}>
              <Link to="/signup" className="block rounded-full border border-rule px-4 py-2 text-sm font-medium transition-colors hover:border-accent hover:text-accent">
                {i.label}
              </Link>
            </motion.div>
          ))}
      ...
      {/* Pricing */}
      <section className="border-t border-rule bg-sunken py-16">
        <div className="mx-auto max-w-6xl px-5">
          <Reveal>
      ...
      <section className="mx-auto max-w-3xl px-5 py-20 text-center">
        ...
        <Stagger stagger={0.05} className="mx-auto mt-6 flex max-w-md flex-wrap justify-center gap-x-5 gap-y-1 text-sm text-ink-faint">
