...
function PreviewMock() {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-[#0b1120] p-4 shadow-2xl shadow-black/50">
  ...
  return (
    <div className="min-h-full bg-white text-slate-900 dark:bg-[#060a13] dark:text-slate-100">
      ...
      <div className="relative overflow-hidden bg-[#060a13] text-white">
        <div className="pointer-events-none absolute -left-24 top-0 h-96 w-96 rounded-full bg-brand-500/20 blur-[120px]" />
        <div className="pointer-events-none absolute -right-16 top-32 h-80 w-80 rounded-full bg-violet-500/20 blur-[120px]" />
      ...
      <section className="mx-auto max-w-6xl px-5 py-16">
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {TRUST.map((t) => (
            <div key={t.title} className="rounded-2xl border border-border p-5 dark:border-white/[0.06]">
      ...
      <section className="border-y border-border bg-surface-tertiary py-16 dark:border-white/[0.06] dark:bg-white/[0.02]">
        <div className="mx-auto max-w-6xl px-5">
          ...
          <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {FEATURES.map((f) => (
              <div key={f.title} className="rounded-2xl border border-border bg-white p-5 dark:border-white/[0.06] dark:bg-slate-900/60">
      ...
      <section className="mx-auto max-w-6xl px-5 py-16">
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          {industries.map((i) => (
            <Link key={i.key} to="/signup" className="rounded-full border border-border px-4 py-2 text-sm font-medium transition-colors hover:border-brand-400 hover:text-brand-600 dark:border-white/10 dark:hover:text-brand-400">
              {i.label}
            </Link>
          ))}
        </div>
      ...
      <section className="mx-auto max-w-3xl px-5 py-20 text-center">
        ...
        <ul className="mx-auto mt-6 flex max-w-md flex-wrap justify-center gap-x-5 gap-y-1 text-sm text-slate-500 dark:text-slate-400">
          {["Free forever plan", "No credit card", "Your data stays private"].map((x) => (
            <li key={x} className="flex items-center gap-1.5"><Check className="h-4 w-4 text-emerald-500" />{x}</li>
          ))}
        </ul>
      </section>
