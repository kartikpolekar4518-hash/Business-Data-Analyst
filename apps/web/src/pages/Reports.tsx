...
export default function Reports() {
  ...
  return (
    <div className="space-y-8">
      ...
      {isError ? <ErrorState message="Could not load reports." retry={() => refetch()} /> : isLoading ? <Spinner /> : !data?.reports.length ? <EmptyState icon={FileText} title="No reports yet" description="Generate an executive report from your latest dataset." /> : (
      ...
      <Modal open={!!viewId} onClose={() => setViewId(undefined)} title={view.data?.report.title ?? "Report"}>
        {view.isError ? <ErrorState message="Could not load this report." retry={() => view.refetch()} /> : !r ? <Spinner /> : (
