  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-8rem)] max-w-3xl flex-col">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div><h1 className="text-2xl font-bold">Chat with your Data</h1><p className="text-sm text-ink-faint">Ask questions in plain English. Answers are computed directly from your dataset.</p></div>
              ...
              <>
                ...
                <div className="absolute right-0 top-full z-50 mt-2 max-h-80 w-72 overflow-y-auto rounded-xl border border-rule bg-surface p-1.5 shadow-dropdown">
                  {conversations.isLoading ? (
                    ...
                    <button key={c.id} onClick={() => openConversation(c.id)} className={cn("flex w-full items-start gap-2.5 rounded-lg px-3 py-2 text-left text-sm transition-colors hover:bg-sunken", c.id === conversationId && "bg-sunken")}>
                      <MessageSquare className="mt-0.5 h-4 w-4 shrink-0 text-ink-faint" />
                      <span className="min-w-0 flex-1"><span className="block truncate font-medium text-ink-soft">{c.title}</span><span className="text-xs text-ink-faint">{timeAgo(c.createdAt)} · {c._count.messages} messages</span></span>
                    </button>
                  )) : <p className="px-3 py-4 text-center text-xs text-ink-faint">No past conversations yet</p>}
                </div>
              </>
            )}
          </div>
      ...
      <div className="flex-1 space-y-4 overflow-y-auto pb-4">
        ...
        {turns.map((t, i) => t.role === "user" ? (
          <div key={i} className="flex justify-end gap-2">
            <div className="max-w-[80%] rounded-2xl rounded-tr-sm bg-accent px-4 py-2 text-sm text-accent-fg">{t.text}</div>
          ...
          <div key={i} className="flex gap-2">
            ...
            <div className="max-w-[85%] space-y-3">
              ...
              {t.result?.metrics && t.result.metrics.length > 0 && !t.result.chart && (
                <div className="flex flex-wrap gap-2">{t.result.metrics.map((m) => <div key={m.label} className="rounded-lg border border-rule bg-surface px-3 py-2"><div className="text-xs text-ink-faint">{m.label}</div><div className="font-semibold">{num(m.value)}</div></div>)}</div>
              )}
              {t.result?.chart && (
