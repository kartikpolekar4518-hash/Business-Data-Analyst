  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-8rem)] max-w-3xl flex-col">
      ...
      <div className="flex-1 space-y-4 overflow-y-auto pb-4">
        ...
        {turns.map((t, i) => t.role === "user" ? (
          ...
          <div key={i} className="flex gap-2">
            ...
            <div className="max-w-[85%] space-y-3">
              <div className="neu-flat rounded-2xl rounded-tl-sm px-4 py-3 text-sm">
                <p>{t.text}</p>
              ...
              {t.result?.metrics && t.result.metrics.length > 0 && !t.result.chart && (
                <div className="flex flex-wrap gap-2">{t.result.metrics.map((m) => <div key={m.label} className="neu-inset rounded-xl px-3 py-2"><div className="text-xs text-slate-500">{m.label}</div><div className="font-semibold">{num(m.value)}</div></div>)}</div>
              )}
              {t.result?.chart && (
        ...
        {loading && <div className="flex gap-2"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-brand-100 text-brand-600 dark:bg-brand-950"><Sparkles className="h-4 w-4 animate-pulse" /></div><div className="neu-flat rounded-2xl px-4 py-3 text-sm text-slate-400">Analyzing…</div></div>}
        <div ref={endRef} />
      </div>
