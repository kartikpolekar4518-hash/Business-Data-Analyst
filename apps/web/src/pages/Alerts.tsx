...
export default function Alerts() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold">Alerts</h1><p className="text-sm text-ink-faint">Automatically flagged risks and anomalies from your data.</p></div>
            ...
            return (
              <Card key={a.id} className={a.read ? "opacity-60" : ""}>
                <CardBody className="flex items-start gap-3 py-3">
                  <div className={`rounded-lg p-2 ${a.severity === "HIGH" ? "bg-sunken text-neg" : a.severity === "MEDIUM" ? "bg-sunken text-warn" : "bg-sunken text-ink-faint"}`}><Icon className="h-5 w-5" /></div>
