...
export const TrendChart = memo(function TrendChart({ data, color = BRAND, ariaLabel = "Trend line chart" }: { data: { label?: string; period?: string; value: number }[]; color?: string; ariaLabel?: string }) {
  const { grid, tick, tooltipStyle } = useAxis();
  const gradientId = useId();
  const norm = data.map((d) => ({ label: d.label ?? d.period, value: d.value }));
  return (
    <div role="img" aria-label={`${ariaLabel}. ${norm.map((d) => `${d.label}: ${d.value}`).join(", ")}`}>
...
export const BarRankChart = memo(function BarRankChart({ data, horizontal = true, ariaLabel = "Ranked bar chart" }: { data: { label: string; value: number }[]; horizontal?: boolean; ariaLabel?: string }) {
  const { grid, tick, tooltipStyle } = useAxis();
  return (
...
export const ForecastChart = memo(function ForecastChart({ history, points, ariaLabel = "Forecast chart with confidence band" }: { history: { period: string; value: number }[]; points: { period: string; value: number; lower: number; upper: number }[]; ariaLabel?: string }) {
  const { grid, tick, tooltipStyle } = useAxis();
  const data = [
    ...history.map((h) => ({ label: h.period, actual: h.value })),
    ...points.map((p) => ({ label: p.period, forecast: p.value, lower: p.lower, upper: p.upper })),
  ];
  const summary = points.map((p) => `${p.period}: ${p.value} (range ${p.lower}–${p.upper})`).join(", ");
  return (
    <div role="img" aria-label={`${ariaLabel}. Projected: ${summary}`}>
