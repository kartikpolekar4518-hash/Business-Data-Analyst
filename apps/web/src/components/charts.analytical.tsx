        ...
        <table className="w-full min-w-[32rem] border-collapse text-sm">
          <thead>
            <tr className="border-b border-rule">
              <th scope="col" className="sticky left-0 z-10 bg-surface py-2 pr-3 text-left text-[11px] font-semibold uppercase tracking-wider text-ink-faint">
                {rowHeaders.join(" › ")}
              </th>
              {columns.map((c) => (
                <th key={c} scope="col" className="py-2 pl-3 text-right">
                  <button
                    ...
                    className="rounded text-[11px] font-semibold uppercase tracking-wider text-ink-faint hover:text-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-accent"
              ...
              <th scope="col" className="py-2 pl-3 text-right text-[11px] font-semibold uppercase tracking-wider text-ink-faint">Total</th>
            </tr>
          </thead>
          <tbody>
            {visible.map((n) => {
            ...
            {!visible.length && (
              <tr><td colSpan={columns.length + 2} className="py-8 text-center text-sm text-ink-faint">No rows to pivot</td></tr>
            )}
          </tbody>
          {showGrandTotal && visible.length > 0 && (
            <tfoot>
              <tr className="border-t-2 border-rule">
                <th scope="row" className="sticky left-0 z-10 bg-surface py-2 pr-3 text-left text-sm font-bold text-ink">Total</th>
                {columns.map((c) => (
...
export function KeyInfluencers({
  ...
  if (!influencers.length) {
    return <p className="py-8 text-center text-sm text-ink-faint">Not enough history to attribute {metric} yet.</p>;
  }
          ...
          <div key={`${inf.factor}-${inf.value}`}>
            <div className="flex items-baseline justify-between gap-2 text-sm">
              ...
              <span className={cn("shrink-0 font-semibold tabular-nums", positive ? "text-pos" : "text-neg")}>
                ...
                {inf.share != null && <span className="ml-1 text-xs font-normal text-ink-faint">{Math.round(inf.share)}%</span>}
              </span>
            </div>
  ...
  if (!fields.length) {
    return (
      <div className="flex flex-col items-center gap-2 py-10 text-center">
        ...
        <p className="text-sm text-ink-faint">No dimensions to break this metric down by.</p>
        <p className="text-xs text-ink-faint">Add a category column to this dataset to use the decomposition tree.</p>
      </div>
    );
  }
      ...
      <div className="flex gap-4 overflow-x-auto pb-2">
        <div className="w-40 shrink-0">
          <div className="rounded-lg border border-accent bg-accent/5 px-3 py-2">
            <div className="text-[11px] font-semibold uppercase tracking-wider text-ink-faint">{rootLabel}</div>
        ...
        {depths.map((depth) => {
          ...
          if (!left.length) {
            return (
              <div key={depth} className="w-40 shrink-0 self-center text-xs text-ink-faint">
          ...
          if (!step) {
            ...
            return (
              <div key={depth} className="w-44 shrink-0">
                <div className="mb-1.5 text-[11px] font-semibold uppercase tracking-wider text-ink-faint">Split by</div>
                <div className="space-y-1">
                  {suggested && (
                    <button
                      type="button"
                      onClick={() => setStep(depth, { field: suggested })}
                      className="w-full rounded-md border border-accent bg-accent/5 px-2.5 py-1.5 text-left text-xs font-medium text-accent hover:bg-accent/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent"
                    >
                  ...
                  {left.map((f) => (
                    <button
                      ...
                      className="w-full rounded-md border border-rule px-2.5 py-1.5 text-left text-xs text-ink-soft hover:bg-sunken focus:outline-none focus-visible:ring-2 focus-visible:ring-accent"
                    >
                      {f.label}
                    </button>
                  ))}
          ...
          return (
            <div key={depth} className="w-52 shrink-0">
              <div className="mb-1.5 flex items-center justify-between gap-2">
                <span className="truncate text-[11px] font-semibold uppercase tracking-wider text-ink-faint">{labelOf(step.field)}</span>
                <button
                  type="button"
                  onClick={() => setStep(depth, null)}
                  className="rounded text-[11px] text-ink-faint hover:text-ink-soft focus:outline-none focus-visible:ring-2 focus-visible:ring-accent"
              ...
              <div className="space-y-1">
                ...
                {hidden > 0 && (
                  <button
                    ...
                    className="w-full rounded-md px-2.5 py-1 text-left text-[11px] font-medium text-accent hover:underline focus:outline-none focus-visible:ring-2 focus-visible:ring-accent"
                  >
                    +{hidden} more
                  </button>
                )}
