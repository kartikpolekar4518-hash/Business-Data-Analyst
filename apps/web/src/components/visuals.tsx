...
export function CardVisual({
  ...
  return (
    <div className="flex h-full flex-col justify-center py-2">
      <div className="text-display leading-none tabular-nums text-ink">
        {formatKpiValue(value, format)}
      </div>
      <div className="mt-2 text-body-sm font-semibold uppercase tracking-wider text-ink-faint">{label}</div>
      {note && <div className="mt-1 text-body-sm text-ink-faint">{note}</div>}
    </div>
  );
}
...
export function KpiVisual({
  ...
  return (
    <div className="flex h-full flex-col justify-center py-2">
      ...
      <div className={cn("mt-2 flex items-center gap-1.5 text-body-sm font-medium tabular-nums", tone)}>
...
export function ImageVisual({
  ...
  return (
    <figure className="m-0">
      ...
      >
        {failed || !src ? (
          <span className="flex flex-col items-center gap-2 text-body-sm text-ink-faint">
      ...
      {caption && <figcaption className="mt-2 text-body-sm text-ink-faint">{caption}</figcaption>}
    </figure>
  );
}
  ...
  return (
    <div>
      <div className="mb-2 flex items-center justify-between gap-2">
        <span className="text-body-sm font-semibold uppercase tracking-wider text-ink-faint">{label}</span>
        {count > 0 && (
          <button
            type="button"
            onClick={onClear}
            className="rounded text-body-sm font-medium text-accent hover:underline focus:outline-none focus-visible:ring-2 focus-visible:ring-accent"
          >
            Clear ({count})
          </button>
        )}
    ...
    <SlicerFrame label={label} count={selected.length} onClear={() => onChange([])}>
      {options.length > 8 && (
        <div className="relative mb-2">
          ...
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search…" className="h-9 pl-8 text-body-sm" aria-label={`Search ${label}`} />
        </div>
      )}
      <div className="space-y-1.5 overflow-y-auto pr-1" style={{ maxHeight }}>
        ...
        {!shown.length && <p className="py-3 text-center text-body-sm text-ink-faint">No matches</p>}
      </div>
    </SlicerFrame>
  );
}
    ...
    return (
      <SlicerFrame label={label} count={range?.min != null || range?.max != null ? 1 : 0} onClear={() => onRangeChange?.({})}>
        <div className="flex items-center gap-2">
          ...
          />
          <span className="text-body-sm text-ink-faint">to</span>
          <Input
        ...
        {invalid && <p className="mt-1.5 text-body-sm text-neg">Minimum is above maximum — nothing will match.</p>}
      </SlicerFrame>
    );
  }
