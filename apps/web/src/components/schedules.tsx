  ...
  return (
    <Card>
      ...
      <CardBody className="space-y-3">
        ...
        {isLoading ? <RowsSkeleton /> : isError ? <ErrorState message="Couldn't load scheduled reports." retry={() => refetch()} /> : !data?.reports.length ? <p className="py-4 text-center text-sm text-ink-faint">No scheduled reports yet.</p> : (
          <div className="divide-y divide-rule">
            {data.reports.map((r) => (
              <div key={r.id} className="flex items-center gap-3 py-2.5">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-ink-faint">Next: {when(r.nextRunAt)}{r.recipients.length ? ` · ${r.recipients.length} recipient(s)` : " · no email"}{r.lastRunAt ? ` · last ${when(r.lastRunAt)}` : ""}</div>
                </div>
  ...
  return (
    <Card>
      ...
      <CardBody className="space-y-3">
        ...
        {isLoading ? <RowsSkeleton /> : isError ? <ErrorState message="Couldn't load alert rules." retry={() => refetch()} /> : !data?.rules.length ? <p className="py-4 text-center text-sm text-ink-faint">No alert rules yet.</p> : (
          <div className="divide-y divide-rule">
            {data.rules.map((r) => (
              <div key={r.id} className="flex items-center gap-3 py-2.5">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-ink-faint">When {r.metric} is {CMP_LABEL[r.comparator] ?? r.comparator} {r.threshold}{r.lastTriggeredAt ? ` · last fired ${when(r.lastTriggeredAt)}` : ""}</div>
                </div>
