...
export function ScheduledReportsSection() {
  ...
  return (
    <Card>
      <CardHeader
        title={<span className="flex items-center gap-2"><CalendarClock className="h-4 w-4 text-accent" />Scheduled reports</span>}
      ...
      <CardBody className="space-y-3">
        {open && editable && (
          <div className="grid gap-3 rounded-xl border border-rule p-3 sm:grid-cols-4">
        ...
        {isLoading ? <RowsSkeleton /> : isError ? <ErrorState message="Couldn't load scheduled reports." retry={() => refetch()} /> : !data?.reports.length ? <p className="py-4 text-center text-body text-ink-faint">No scheduled reports yet.</p> : (
          <div className="divide-y divide-rule-soft">
            {data.reports.map((r) => (
              <div key={r.id} className="flex items-center gap-3 py-2.5">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2"><span className="truncate text-body font-medium">{r.title || "Latest dataset report"}</span><Badge tone="blue">{r.frequency.toLowerCase()}</Badge>{r.lastRunStatus === "error" && <Badge tone="red">last run failed</Badge>}</div>
                  <div className="text-body-sm text-ink-faint">Next: {when(r.nextRunAt)}{r.recipients.length ? ` · ${r.recipients.length} recipient(s)` : " · no email"}{r.lastRunAt ? ` · last ${when(r.lastRunAt)}` : ""}</div>
                </div>
...
export function AlertRulesSection() {
  ...
  return (
    <Card>
      <CardHeader
        title={<span className="flex items-center gap-2"><BellPlus className="h-4 w-4 text-accent" />Alert rules</span>}
      ...
      <CardBody className="space-y-3">
        ...
        {isLoading ? <RowsSkeleton /> : isError ? <ErrorState message="Couldn't load alert rules." retry={() => refetch()} /> : !data?.rules.length ? <p className="py-4 text-center text-body text-ink-faint">No alert rules yet.</p> : (
          <div className="divide-y divide-rule-soft">
            {data.rules.map((r) => (
              <div key={r.id} className="flex items-center gap-3 py-2.5">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2"><span className="truncate text-body font-medium">{r.name}</span><Badge tone="slate">{r.frequency.toLowerCase()}</Badge></div>
                  <div className="text-body-sm text-ink-faint">When {r.metric} is {CMP_LABEL[r.comparator] ?? r.comparator} {r.threshold}{r.lastTriggeredAt ? ` · last fired ${when(r.lastTriggeredAt)}` : ""}</div>
                </div>
