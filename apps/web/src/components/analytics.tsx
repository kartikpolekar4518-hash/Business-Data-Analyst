    ...
    <Card className={className}>
      ...
      <CardBody>
        {isLoading ? <Spinner /> : isError ? <ErrorState message="Couldn't load the breakdown." retry={() => refetch()} />
          : !data || !data.drivers.length ? <p className="py-6 text-center text-sm text-ink-faint">Not enough history to attribute the change yet.</p>
          : (() => {
            const max = Math.max(...data.drivers.map((d) => Math.abs(d.contribution)), 1);
            return (
              <div className="space-y-3">
                <div className="flex items-baseline gap-2 text-sm">
                  <span className={cn("text-lg font-bold", up ? "text-pos" : "text-neg")}>
                    {up ? "+" : "−"}{money(Math.abs(data.totalChange))}
                  </span>
                  <span className="text-ink-faint">{money(data.totalPrevious)} → {money(data.totalCurrent)}{data.totalChangePct !== null ? ` (${data.totalChangePct}%)` : ""}</span>
                </div>
                {data.drivers.map((d) => {
                    ...
                    <div key={d.label}>
                      <div className="flex items-center justify-between gap-2 text-sm">
                        ...
                        <span className={cn("shrink-0 font-semibold", pos ? "text-pos" : "text-neg")}>
                          {pos ? "+" : "−"}{money(Math.abs(d.contribution))}{d.shareOfChange !== null ? <span className="ml-1 text-xs font-normal text-ink-faint">{d.shareOfChange}%</span> : null}
                        </span>
                      </div>
                ...
                {data.otherCount > 0 && (
                  <div className="flex items-center justify-between gap-2 border-t border-rule pt-2 text-sm text-ink-faint">
    ...
    <Card className={className}>
      ...
      <CardBody>
        {isLoading ? <Spinner /> : isError ? <ErrorState message="Couldn't check for anomalies." retry={() => refetch()} />
          : !data || !data.anomalies.length ? <p className="py-6 text-center text-sm text-ink-faint">No unusual periods detected — the trend is stable.</p>
          : (
            <div className="space-y-2">
              {data.anomalies.map((a) => (
                <div key={a.period} className="flex items-start gap-3 rounded-lg border border-rule p-3">
                  <div className={cn("rounded-lg p-1.5", a.direction === "spike" ? "bg-sunken text-pos" : "bg-sunken text-neg")}>
                    {a.direction === "spike" ? <TrendingUp className="h-4 w-4" /> : <TrendingDown className="h-4 w-4" />}
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-ink-soft">{a.period}</span>
                      <Badge tone={sevTone(a.severity)}>{a.severity}</Badge>
                    </div>
                    <p className="mt-0.5 text-[13px] text-ink-soft">{a.reason}</p>
                  </div>
                </div>
              ))}
            </div>
    ...
    <Card className={className}>
      ...
      <CardBody>
        {isLoading ? <Spinner /> : isError ? <ErrorState message="Couldn't load segments." retry={() => refetch()} />
          : !data || !data.segments.length ? <p className="py-6 text-center text-sm text-ink-faint">Not enough data to segment yet.</p>
          : (
            <div className="grid gap-3 sm:grid-cols-3">
              {data.segments.map((s) => (
                <div key={s.key} className="rounded-xl border border-rule p-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-semibold text-ink">{s.label}</span>
                    <Badge tone={s.key === "high" ? "blue" : s.key === "mid" ? "amber" : "slate"}>{s.count}</Badge>
                  </div>
                  <div className="mt-1 text-xs text-ink-faint">{s.shareOfRevenue}% of revenue · avg {money(s.avgRevenue)}</div>
                  <p className="mt-2 text-[11px] leading-snug text-ink-faint">{s.rule}</p>
                </div>
              ))}
            </div>
          )}
    ...
    <Card className={className}>
      ...
      <CardBody>
        {isLoading ? <Spinner /> : isError ? <ErrorState message="Couldn't load correlations." retry={() => refetch()} />
          : !shown.length ? <p className="py-6 text-center text-sm text-ink-faint">No meaningful correlations between numeric columns.</p>
          : (
            <div className="space-y-2">
              {shown.map((p) => (
                <div key={`${p.a}-${p.b}`} className="flex items-center justify-between gap-2 rounded-lg border border-rule px-3 py-2">
                  <div className="min-w-0">
                    <div className="truncate text-sm font-medium text-ink-soft">{p.a} ↔ {p.b}</div>
                    <div className="text-[11px] text-ink-faint">{p.interpretation}</div>
                  </div>
                  <div className="shrink-0 text-right">
                    <div className={cn("text-sm font-semibold tabular-nums", p.direction === "positive" ? "text-pos" : "text-neg")}>
              ...
              <p className="pt-1 text-[11px] text-ink-faint">{data!.caveat}</p>
            </div>
          )}
      </CardBody>
    </Card>
