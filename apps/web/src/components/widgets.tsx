          ...
          >
            <div className="flex items-center justify-between gap-2 border-b border-rule px-4 py-2.5">
              <div className="flex min-w-0 items-center gap-2">
                ...
                <h3 className="truncate text-body font-semibold text-ink">{item.title}</h3>
              </div>
              <Dropdown
                align="right"
                trigger={<button className="flex h-7 w-7 items-center justify-center rounded-md text-ink-faint hover:bg-sunken hover:text-ink" aria-label="Widget actions"><MoreHorizontal className="h-4 w-4" /></button>}
              >
                {(close) => (
                  <div className="w-44">
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between gap-2">
        <span className="text-body-sm font-medium text-ink-soft">{label}</span>
        <span className={cn("text-body font-semibold tabular-nums", done ? "text-pos" : "text-ink")}>{pct}%</span>
      </div>
      ...
      <div className="mt-2 flex items-center justify-between text-body-sm text-ink-faint tabular-nums">
