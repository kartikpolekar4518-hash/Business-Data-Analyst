...
const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
  <section className="border-t border-rule pt-3">
    <h4 className="label mb-1.5 text-ink-faint">{title}</h4>
    {children}
  </section>
);

const Mono = ({ children }: { children: React.ReactNode }) => (
  <code className="rounded bg-sunken px-1.5 py-0.5 font-mono text-body-sm text-ink">{children}</code>
);
...
function Panel({ data }: { data: Explanation }) {
  ...
  return (
    <div className="space-y-3 text-body-sm">
      <div>
        <div className="text-body-sm font-semibold uppercase tracking-wider text-ink-faint">{metric.label}</div>
        <div className="text-heading-1 font-bold tabular-nums text-ink">{formatKpiValue(metric.value, metric.format as KpiFormat)}</div>
      </div>
      ...
      <Section title="Source">
        {formula.sources.length ? formula.sources.map((s) => (
          <div key={s.column} className="mb-0.5">
            <Mono>{s.column}</Mono>
            {s.detectedBy && <span className="ml-2 text-body-sm text-ink-faint">detected using {s.detectedBy}</span>}
          </div>
      ...
      <Section title="Data">
        ...
        {inputs.exclusions.length > 0 && (
          <ul className="mt-1 space-y-0.5 text-body-sm text-ink-faint">
            {inputs.exclusions.map((e) => (
              <li key={e.reason}>{num(e.count)} {EXCLUSION_LABEL[e.reason] ?? e.reason}</li>
            ))}
          </ul>
        )}
        <p className="mt-1 text-body-sm text-ink-faint">{inputs.note}</p>
      </Section>
      ...
      <Section title="Comparison">
        {comparison.basis === "unavailable" ? (
          ...
          <>
            ...
            <p className="mt-1 text-body-sm text-ink-faint">{comparison.description}</p>
          </>
        )}
      </Section>
      ...
      {drivers && drivers.drivers.length > 0 && (
        <Section title="What moved it">
          ...
          {drivers.reconciled && (
            <div className="mt-1 border-t border-dashed border-rule pt-1 text-body-sm font-medium text-pos">
      ...
      <Section title="Provenance">
        <dl className="space-y-0.5 text-body-sm text-ink-faint">
        ...
        {provenance.cleaning.length > 0 && (
          <div className="mt-1.5 text-body-sm text-ink-faint">
      ...
      <p className="border-t border-rule pt-2 text-body-sm text-ink-faint">
