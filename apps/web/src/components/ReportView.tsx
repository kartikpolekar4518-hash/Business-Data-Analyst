...
const Kpi = ({ label, value }: { label: string; value: string }) => <div className="rounded-lg bg-sunken p-2"><div className="text-xs text-ink-faint">{label}</div><div className="font-semibold">{value}</div></div>;
      ...
      <section><h4 className="mb-1 font-semibold">Risks &amp; Recommendations</h4>
        {r.recommendations.map((rec, i) => (
          <div key={i} className="mb-2 rounded-lg border border-rule-soft p-2">
            ...
            <p className="mt-1 text-xs text-ink-faint">{rec.observation}</p>
            <p className="mt-0.5 text-xs text-accent">{rec.action}</p>
          </div>
        ))}
      </section>
