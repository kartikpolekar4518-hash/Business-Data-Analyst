  ...
  return (
    <div className="mx-auto max-w-3xl py-8">
      <div className="text-center">
        ...
        <p className="mx-auto mt-2 max-w-md text-body text-ink-faint">
          {canUpload
          ...
          <Link to="/data" className="group">
            <Card hoverable className="h-full">
              <CardBody className="flex h-full flex-col">
                ...
                <p className="mt-1 flex-1 text-body text-ink-faint">
                  Drag in a CSV or Excel file. We profile and quality-check it on the way in.
                </p>
                <span className="mt-3 inline-flex items-center gap-1 text-body font-medium text-accent">
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-3">
        {steps.map((s) => (
          <div key={s.title} className="text-center">
            ...
            <h4 className="mt-2 text-body font-semibold text-ink">{s.title}</h4>
            <p className="mt-1 text-body-sm text-ink-faint">{s.body}</p>
          </div>
        ))}
      </div>
    </div>
    ...
    <Card>
      <CardBody>
        <div className="flex items-start justify-between gap-3">
          <div>
            <h3 className="text-heading-3 font-semibold text-ink">Get the most out of NoPS</h3>
            <p className="mt-0.5 text-body-sm text-ink-faint">{done} of {steps.length} done</p>
          </div>
          <button
            onClick={onDismiss}
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2">
          {steps.map((s) => (
            <Link
              ...
              to={s.to}
              className={cn(
                "flex items-center gap-3 rounded-lg border px-3 py-2.5 text-body transition-colors",
                s.done
                  ? "border-pos/30 bg-emerald-50/60"
            ...
            >
              <span
                className={cn(
                  ...
                  s.done ? "bg-pos text-canvas" : "bg-sunken text-ink-faint",
                )}
              >
...
const TOUR = [
  ...
  { icon: MessagesSquare, title: "Ask questions & report", body: "Ask in plain English, forecast ahead, and export a board-ready PDF — all deterministic auditable." },
];
  ...
  return (
    <Modal open={open} onClose={onClose} title="Welcome to NoPS">
      <div className="text-center">
        ...
        <p className="mx-auto mt-1 max-w-xs text-body text-ink-faint">{s.body}</p>
      </div>

      <div className="mt-5 flex items-center justify-center gap-1.5">
        {TOUR.map((_, i) => (
          <span key={i} className={cn("h-1.5 rounded-full transition-all", i === step ? "w-5 bg-accent" : "w-1.5 bg-rule-strong")} />
        ))}
      </div>
