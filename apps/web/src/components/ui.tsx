...
const variants: Record<Variant, string> = {
  primary:
    "bg-gradient-to-br from-brand-500 to-brand-700 text-white shadow-sm shadow-brand-700/25 hover:from-brand-600 hover:to-brand-800 active:scale-[0.98]",
  secondary:
...
>(({ variant = "primary", loading, className, children, disabled, ...props }, ref) => (
  <button
    ...
    className={cn(
      "inline-flex items-center justify-center gap-2 rounded-lg px-3.5 py-2 text-[13.5px] font-medium transition-all duration-150 disabled:opacity-50 disabled:pointer-events-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-[#09090b]",
      variants[variant],
      className
    )}
    {...props}
...
export const Card = ({ className, children }: { className?: string; children: ReactNode }) => (
  <div
    className={cn(
      ...
      "dark:border-white/[0.07] dark:bg-[#111118] dark:shadow-kpi-glow",
      className
    )}
  >
    {children}
  ...
  return (
    ...
    >
      <div
        ...
        aria-modal="true"
        aria-label={title}
        className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl animate-in dark:border-white/10 dark:bg-[#141420]"
      ...
      <div aria-live="polite" className="fixed bottom-5 right-5 z-[60] flex flex-col gap-2">
        {toasts.map((t) => {
          ...
          return (
            <div
              key={t.id}
              className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white/95 px-4 py-3 text-[13px] shadow-xl shadow-black/[0.08] backdrop-blur-md animate-in dark:border-white/10 dark:bg-[#1a1a24]/95"
            >
              <Icon
                className={cn(
                  "h-4 w-4 shrink-0",
