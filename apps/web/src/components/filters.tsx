...
export const RelativeDateSelect = ({ onSelect }: { onSelect: (r: Range) => void }) => (
  <Dropdown
    trigger={
      <button className="inline-flex h-10 items-center gap-2 rounded-lg border border-rule bg-surface px-3 text-body text-ink-soft transition hover:bg-sunken">
    ...
    <div className="inline-flex h-10 items-center rounded-lg border border-rule bg-surface p-1">
      {COMPARISONS.map((c) => (
        <button
          ...
          className={cn(
            "rounded-md px-3 py-1.5 text-body transition",
            value === c.value
    ...
    <Dropdown
      ...
      trigger={
        <button
          type="button"
          className={cn(
            "inline-flex h-10 w-full items-center justify-between gap-2 rounded-lg border border-rule bg-surface px-3 text-body transition hover:bg-sunken",
    ...
    >
      <div className="w-64">
        <div className="mb-1 flex items-center justify-between px-1">
          <span className="text-body-sm font-semibold uppercase tracking-wide text-ink-faint">{label}</span>
          {selected.length > 0 && (
            <button onClick={() => onChange([])} className="text-body-sm font-medium text-accent hover:underline">Clear</button>
          )}
        </div>
        {options.length > 8 && (
          <div className="relative mb-1">
            ...
            <input
              ...
              placeholder="Search…"
              className="h-8 w-full rounded-lg border border-rule bg-surface pl-8 pr-2 text-body outline-none focus:border-accent"
            />
          </div>
        )}
        <div className="max-h-56 overflow-y-auto">
          {shown.length === 0 ? (
            <p className="px-2 py-3 text-center text-body-sm text-ink-faint">No matches</p>
          ) : (
            shown.map((o) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => toggle(o)}
                  className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left text-body text-ink-soft hover:bg-sunken"
                >
                  <span className={cn(
...
export const FilterChips = ({ chips, onClearAll }: { chips: Chip[]; onClearAll: () => void }) => {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      {chips.map((c) => (
        <span key={c.id} className="inline-flex items-center gap-1.5 rounded-full border border-rule bg-surface px-2.5 py-1 text-body-sm text-ink-soft">
          {c.label}
          <button onClick={c.onRemove} aria-label="Remove filter" className="text-ink-faint transition-colors hover:text-ink">
            <X className="h-3.5 w-3.5" />
          </button>
        </span>
      ))}
      <button onClick={onClearAll} className="text-body-sm font-medium text-ink-faint underline-offset-2 hover:text-ink hover:underline">
        Clear all
      </button>
    </div>
  );
    ...
    <>
      ...
      >
        {(close) => (
          <div className="w-60">
            {views.length === 0 ? (
              <p className="px-2 py-3 text-center text-body-sm text-ink-faint">No saved views yet</p>
            ) : (
              views.map((v) => (
                <div key={v.id} className="group flex items-center rounded-lg hover:bg-sunken">
                  <button onClick={() => { onApply(v.query); close(); }} className="flex-1 truncate px-2.5 py-1.5 text-left text-body text-ink-soft">
                    {v.name}
                  </button>
      ...
      <Modal open={saving} onClose={() => setSaving(false)} title="Save current view">
        ...
        <p className="mt-2 text-body-sm text-ink-faint">Everyone in your organisation can use this view. Saving over an existing name replaces it.</p>
